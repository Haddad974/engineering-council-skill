...
2. **Scans context** — CLAUDE.md, memory files, ADRs, attached datasheets/schematics/code — and takes a live **tool inventory** of your connected tools
3. **Convenes 6 advisors in parallel:**
   - **The Failure Analyst** — FMEA mindset; hunts the failure mode everyone ignores
...
5. **Chairman synthesis** — agreement, clashes, blind spots, risk register, verification ledger (volatile facts like prices/EOL/versions get web-verified, never trusted), recommendation, tooling plan, one first step, and an ADR draft when the decision is architectural
6. **Outputs** a visual HTML report + full markdown transcript
